...
nm_list = ['009','010','011','012','013','014','015','016']
...
while True: 
    ...
    if msg == '테스트시작':        
        ...
        try:
            ...
            driver1.save_screenshot(r'c:\\qa\\'+nnow+'_client3.png') #선택 클라이언트 클릭
            ## 파일 올리기 
            urll = 'http://192.168.0.37:8000/fileUpload'
            files = {'file1': open(r'c:\\qa\\'+nnow+'_client3.png', 'rb')}
        ...
        if xy(msg) != '0': #DB에 명령어가 있을 경우 진행
            ...
            if len(msg.split(' ')) >= 2:                
                if msg.split(' ')[0] =='ㅋ': #개별 클라 실행
                    ...
                    s_msg('명령어 목록이 없습니다.(2)')                        
            else:
                s_msg('명령어 목록이 없습니다.(1)')
